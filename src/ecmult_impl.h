...
static void
secp256k1_ecmult_context_build(secp256k1_ecmult_context *ctx, const secp256k1_callback *cb) {
    secp256k1_gej gj;
    ...
    ctx->pre_g = (secp256k1_ge_storage (*)[]) checked_malloc(cb,
                                                             sizeof((*ctx->pre_g)[0])
...
static void secp256k1_ecmult_strauss_wnaf(const secp256k1_ecmult_context *ctx,
    ...
    if (no > 0) {
        /* Compute the odd multiples in Jacobian form. */
        secp256k1_ecmult_odd_multiples_table(ECMULT_TABLE_SIZE(WINDOW_A),
                                             state->prej,
                                             state->zr,
...
#endif
            secp256k1_gej_rescale(&tmp,
                                  &(state->prej[(np - 1) * ECMULT_TABLE_SIZE(WINDOW_A)
                                      + ECMULT_TABLE_SIZE(WINDOW_A) - 1].z));
            secp256k1_ecmult_odd_multiples_table(ECMULT_TABLE_SIZE(WINDOW_A),
                                                 state->prej + np * ECMULT_TABLE_SIZE(WINDOW_A),
                                                 state->zr + np * ECMULT_TABLE_SIZE(WINDOW_A),
                                                 &tmp);
            secp256k1_fe_mul(state->zr + np * ECMULT_TABLE_SIZE(WINDOW_A),
                             state->zr + np * ECMULT_TABLE_SIZE(WINDOW_A),
                             &(a[state->ps[np].input_pos].z));
        }
        /* Bring them to the same Z denominator. */
        secp256k1_ge_globalz_set_table_gej(ECMULT_TABLE_SIZE(WINDOW_A) * no,
                                           state->pre_a,
                                           &Z,
                                           state->prej,
                                           state->zr);
...
static size_t secp256k1_strauss_scratch_size(size_t n_points) {
    ...
    static const size_t point_size =
        (sizeof(secp256k1_ge) + sizeof(secp256k1_gej) + sizeof(secp256k1_fe))
            * ECMULT_TABLE_SIZE(WINDOW_A) + sizeof(struct secp256k1_strauss_point_state)
...
static int secp256k1_ecmult_strauss_batch(const secp256k1_ecmult_context *ctx,
    ...
    if (!secp256k1_scratch_allocate_frame(scratch,
                                          secp256k1_strauss_scratch_size(n_points),
                                          STRAUSS_SCRATCH_OBJECTS)) {
        return 0;
    }
    points = (secp256k1_gej *) secp256k1_scratch_alloc(scratch, n_points * sizeof(secp256k1_gej));
    scalars =
        (secp256k1_scalar *) secp256k1_scratch_alloc(scratch, n_points * sizeof(secp256k1_scalar));
    state.prej = (secp256k1_gej *) secp256k1_scratch_alloc(scratch,
                                                           n_points * ECMULT_TABLE_SIZE(WINDOW_A)
                                                               * sizeof(secp256k1_gej));
    state.zr = (secp256k1_fe *) secp256k1_scratch_alloc(scratch,
                                                        n_points * ECMULT_TABLE_SIZE(WINDOW_A)
                                                            * sizeof(secp256k1_fe));
#ifdef USE_ENDOMORPHISM
...
#else
    state.pre_a = (secp256k1_ge *) secp256k1_scratch_alloc(scratch,
                                                           n_points * ECMULT_TABLE_SIZE(WINDOW_A)
                                                               * sizeof(secp256k1_ge));
#endif
    state.ps = (struct secp256k1_strauss_point_state *) secp256k1_scratch_alloc(scratch,
                                                                                n_points
...
static size_t secp256k1_strauss_max_points(secp256k1_scratch *scratch) {
    return secp256k1_scratch_max_allocation(scratch, STRAUSS_SCRATCH_OBJECTS)
...
static int secp256k1_ecmult_pippenger_wnaf(secp256k1_gej *buckets,
    ...
    for (np = 0; np < num; ++np) {
        ...
        state->ps[no].input_pos = np;
        state->ps[no].skew_na =
            secp256k1_wnaf_fixed(&state->wnaf_na[no * n_wnaf], &sc[np], bucket_window + 1);
        no++;
    }
    secp256k1_gej_set_infinity(r);
...
static size_t secp256k1_pippenger_scratch_size(size_t n_points, int bucket_window) {
    ...
    return ((1 << bucket_window) * sizeof(secp256k1_gej) + sizeof(struct secp256k1_pippenger_state)
        + entries * entry_size);
}
...
static int secp256k1_ecmult_pippenger_batch(const secp256k1_ecmult_context *ctx,
    ...
    if (!secp256k1_scratch_allocate_frame(scratch,
                                          secp256k1_pippenger_scratch_size(n_points, bucket_window),
    ...
    state_space =
        (struct secp256k1_pippenger_state *) secp256k1_scratch_alloc(scratch, sizeof(*state_space));
    state_space->ps = (struct secp256k1_pippenger_point_state *) secp256k1_scratch_alloc(scratch,
                                                                                         entries
                                                                                             * sizeof(*state_space
                                                                                                 ->ps));
    state_space->wnaf_na = (int *) secp256k1_scratch_alloc(scratch,
                                                           entries * (WNAF_SIZE(bucket_window + 1))
                                                               * sizeof(int));
    buckets =
        (secp256k1_gej *) secp256k1_scratch_alloc(scratch, (1 << bucket_window) * sizeof(*buckets));
...
#endif
        space_overhead = ((1 << bucket_window) * sizeof(secp256k1_gej) + entry_size
...
typedef int(*secp256k1_ecmult_multi_func)(const secp256k1_ecmult_context *,
                                          secp256k1_scratch *,
                                          secp256k1_gej *,
...
static int secp256k1_ecmult_multi_var(const secp256k1_ecmult_context *ctx,
    ...
    int (*f)(const secp256k1_ecmult_context *,
             secp256k1_scratch *,
             secp256k1_gej *,
             const secp256k1_scalar *,
             secp256k1_ecmult_multi_callback cb,

...
unsigned char *hexstr_to_char(const char *hexstr) {
    ...
    for (size_t i = 0, j = 0; j < final_len; i += 2, j++) {
        chrs[j] = static_cast<unsigned char>((hexstr[i] % 32 + 9) % 25 * 16 + (hexstr[i + 1] % 32 + 9) % 25);
    }

    return chrs;
}

void bytesToHex(unsigned char *data, char *out, size_t len) {
...
//    char ssString[65];
//    char outString[129];
//    bytesToHex(reinterpret_cast<unsigned char *>(r), rString, 32);
//    bytesToHex(reinterpret_cast<unsigned char *>(sig.data), rrString, 32);
    bytesToHex(reinterpret_cast<unsigned char *>(s), sString, 32);
//    bytesToHex(reinterpret_cast<unsigned char *>(sig.data + 32), ssString, 32);
//    bytesToHex(reinterpret_cast<unsigned char *>(outputSer), outString, 64);

    env->SetByteArrayRegion(rOut, 0, 32, r);
    env->SetByteArrayRegion(sOut, 0, 32, s);
    env->SetByteArrayRegion(vOut, 0, 1, v);

    jobjectArray retArray = (env)->NewObjectArray(3, (env)->FindClass("[B"), NULL);
    env->SetObjectArrayElement(retArray, 0, rOut);
    env->SetObjectArrayElement(retArray, 1, sOut);
    env->SetObjectArrayElement(retArray, 2, vOut);
